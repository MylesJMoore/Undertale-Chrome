if (global.player_can_move) {
	...
	#region Open Inventory Menu - Keyboard
	//Check if "Start" button is pressed on the controller for confirmation
	if (!global.player_triggered_battle && keyboard_check_pressed(ord("C")) && !place_meeting(x, y, obj_saveblock))
	{
	    TurnOnPlayerMenu();
	}
	#endregion
	...
	#region Open Inventory Menu - Controller
	//Check if "Start" button is pressed on the controller for confirmation
	if (!global.player_triggered_battle && gamepad_button_check_pressed(0, gp_start) && !place_meeting(x, y, obj_saveblock))
	{
	    TurnOnPlayerMenu();
	}
	#endregion
