...
//Automatic Title - Check if Dialogue is done typing then progress based on that counter
if(global.dialogue_is_done_typing) {
	if(dialogue_text_delay > 0) {
		dialogue_text_delay--;
	}
}

//Automatically Go to next frame of Intro
if(dialogue_text_delay == 0)
{
	...
	if (title_index < _total_frames) {
		...
		//Go to the next room (Undertale Chrome Title)
		room_goto_next();
	}
}
